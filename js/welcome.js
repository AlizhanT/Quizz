...
async function updateAuthUI() {
    ...
    if (session) {
        // User is logged in - redirect to saved-quizzes or show profile (only on welcome page)
        if (isWelcomePage) {
            // Redirect logged-in users directly to saved-quizzes
...
function showUserProfile(user) {
    ...
    if (!userProfile) {
        ...
        userProfile.innerHTML = `
            ...
            <button class="nav-btn" onclick="goToSavedQuizzes()" data-translate="navigation.createNewQuiz">${t('navigation.createNewQuiz')}</button>
            <button class="nav-btn" onclick="logout()" data-translate="auth.logout">${t('auth.logout')}</button>
        `;
